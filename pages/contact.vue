<template>
    <h1 class="my-4 text-2xl text-cyan-700 darl:cyan-500 font-medium">Контакты</h1>
    <p class="my-4 text-black dark:text-white">«Чашка кофе LEBO собирает близких за одним столом, чтобы вместе открыть богатство общения»</p>
    <h2 class="my-4 text-black text-xl font-medium dark:text-white">Контакты:</h2>
    <ul class="my-4 text-black dark:text-white">
        <li>Номер: <a class="underline hover:text-cyan-500" href="tel:79933079829">+7 9282947307</a></li>
        <li>Почта: <a class="underline hover:text-cyan-500" href="mailto:oleg@nastyle.pro">pulse_sport@mail.ru</a></li>
    </ul>
    <h2 class="my-4 text-black text-xl font-medium dark:text-white">Форма обратной связи:</h2>
    <AppFeedback />
</template>

<script setup>
const index = useIndexStore();
const seo = ref({});

// Загружаем данные с SSR
const { data } = await useAsyncData('seo', () => 
  $fetch('https://908bd2082661.vps.myjino.ru/api/contact?populate=*'),
  { server: true }
);

// Заполняем seo после загрузки
if (data.value?.data?.seo) {
  seo.value = data.value.data.seo;
}

// Используем useHead после загрузки данных
useHead({
  title: () => seo.value?.metaTitle ? `${seo.value.metaTitle} | Lebbo` : 'Lebbo',
  meta: [
    { name: 'description', content: () => seo.value?.metaDescription || '' }
  ],
});
</script>