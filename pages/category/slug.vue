<template>
  ...
  const fetch = async () => {
    try {
        ...
        const res = await $fetch(`https://localhost:1337/api/posts?filters[slug][$eqi]=${slug}&filters[categories][slug][$eqi]=${category}&populate=*`);
  ...
  const updateViews = async (documentId) => {
    try {
      await $fetch(`https://localhost:1337/api/posts/${documentId}`, {
        method: 'PUT',
        body: {
          data: {
